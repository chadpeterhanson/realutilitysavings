...
# --- regression tests for the controlled-load / zero-supply bug -------------

def test_controlled_load_picks_main_period():
    """CL period listed first must NOT be taken as the main tariff."""
    from eme_loader import map_plan_detail
    plan = {"planId": "CL1", "displayName": "Alinta CL First",
            "geography": {"includedPostcodes": ["5000-5999"]},
            "electricityContract": {"tariffPeriod": [
                {"displayName": "Controlled Load", "rateBlockUType": "singleRate",
                 "dailySupplyCharges": "0", "singleRate": {"rates": [{"unitPrice": "22.00"}]}},
                {"displayName": "Single Rate", "rateBlockUType": "singleRate",
                 "dailySupplyCharges": "95.70", "singleRate": {"rates": [{"unitPrice": "40.15"}]}}],
                "solarFeedInTariff": [{"singleTariff": {"rates": [{"unitPrice": "4.00"}]}}]}}
    p, _ = map_plan_detail(plan)
    assert abs(p.supply - 0.957) < 1e-3, p.supply
    assert abs(p.flat - 0.4015) < 1e-3, p.flat
    print(f"PASS  controlled-load plan picks main period (supply ${p.supply:.3f}, rate ${p.flat})")


def test_plausibility_filter_drops_impossible():
    """A plan that parses to $0 supply must be excluded, not shown."""
    from eme_loader import load_plans_from_details
    bad = {"planId": "BAD", "displayName": "Broken",
           "geography": {"includedPostcodes": ["5000-5999"]},
           "electricityContract": {"tariffPeriod": [
               {"displayName": "X", "rateBlockUType": "singleRate",
                "singleRate": {"rates": [{"unitPrice": "40.00"}]}}]}}  # no supply
    good = {"planId": "GOOD", "displayName": "Fine",
            "geography": {"includedPostcodes": ["5000-5999"]},
            "electricityContract": {"tariffPeriod": [
                {"displayName": "X", "rateBlockUType": "singleRate",
                 "dailySupplyCharges": "98.00", "singleRate": {"rates": [{"unitPrice": "38.00"}]}}],
                "solarFeedInTariff": [{"singleTariff": {"rates": [{"unitPrice": "5.00"}]}}]}}
    plans, notes = load_plans_from_details([{"data": bad}, {"data": good}])
    assert len(plans) == 1, [p.name for p in plans]
    assert plans[0].name.endswith("Fine") or "Fine" in plans[0].name
